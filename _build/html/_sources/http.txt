...
Set to ``0``: Test if ATS buffers the request until the post body has been received and then send the request to origin.
...
Set to ``1``: Test if ATS sends a ``408 Request Timeout`` response when a ``POST`` request timeout happens.
